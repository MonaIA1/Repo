...
def process_multi(aligned_file_rgb_prefixes):
    global BASE_PATH, DEST_PATH, DEVICE, THREADS, to_process, proc_prefixes, total_time, processed
    
    file_prefix, rgb_prefix, labels2D_prefixes = aligned_file_rgb_prefixes
    ...
    if np.sum(segmentation_label) == 0:
     ...
     ##########################################################################################################
      ...
      np.savez_compressed(preproc_file,
                           ...
                           mapping = depth_mapping.astype(np.int64),
                           rgb = rgb_prefix, # rgb image
                           label2d = labels2D_prefixes) # 2d segmentation map
...
def Run():
    ...
    depth_prefixes,rgb_prefixes, labels2D_prefixes  = get_file_prefixes_from_path(data_path)
    print(f'RGB prefixes found: {len(rgb_prefixes)}')
    print(" ..")

    
    # Ensure the rgb_prefixes and file_prefixes are aligned correctly
    
    aligned_file_rgb_prefixes = [(depth_prefixes[num], rgb_prefixes[num], labels2D_prefixes[num]) for num in depth_prefixes.keys() if num in rgb_prefixes]
    
    print (f'aligned_file_rgb_prefixes: {len (aligned_file_rgb_prefixes)}')
    #print (f'{aligned_file_rgb_prefixes}')
    
    to_process = len(aligned_file_rgb_prefixes)
    total_time = time.time()
 
    if to_process>0:
        pool = multiprocessing.Pool(processes=THREADS) 
        pool.map(process_multi, aligned_file_rgb_prefixes)
        pool.close()
        pool.join()
