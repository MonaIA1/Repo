...
def process(file_prefix, voxel_shape, down_scale = 4):
    ...
    #segmentation_label_fullscale = np.zeros(num_voxels, dtype=np.int32)
    ...
    _lib.Process(ctypes.c_char_p(bytes(file_prefix+'.bin','utf-8')),
                      ...
                      #segmentation_label_fullscale.ctypes.data_as(ctypes.c_void_p)
                      
                      ) 
    ##########################################################################                                 
    return vox_tsdf, segmentation_label, vox_weights, vox_masks, depth_mapping
    #return vox_tsdf, segmentation_label, vox_weights, vox_masks, cam_pose, segmentation_label_fullscale    # to return the the cam_pose values and the full scaled labels (240,144,240) 
    #########################################################################
