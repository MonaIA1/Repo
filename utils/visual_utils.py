    ...
    for x in range(shape[0]):
        for y in range(shape[1]):
            for z in range(shape[2]):
                ...
                coord_y.append(y)
                coord_z.append(z)
                voxel_v.append(vox[x,y,z].item())  # .item() to get the value as a standard Python number
                count += 1
...
def write_header(obj_file, mtl_file, name):
    obj_file.write("# MDBNet Wavefront obj exporter v1.0\n")
    obj_file.write("mtllib %s.mtl\n" % os.path.basename(name))
    obj_file.write("o Cube\n")

    mtl_file.write("# MDBNet Wavefront obj exporter v1.0\n")
